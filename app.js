// AApni details yahan fill karein
const GITHUB_TOKEN = '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';
const OWNER = 'Mrv0921'; 
...
function openModal(seatNo) {
    ...
    // Check agar seat pehle se book hai
    const existingBooking = libraryData.find(b => b.seatNo == seatNo);
    
    if (existingBooking) {
        document.getElementById('panel-title').innerText = `Update Seat: ${seatNo}`;
        ...
        delBtn.classList.remove('hidden'); // Show clear button
    } else {
        form.reset();
        document.getElementById('panel-title').innerText = `Book Seat: ${seatNo}`;
